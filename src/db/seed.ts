...
async function seed() {
  ...
  const [lagavulin] = await db.insert(distilleries).values([
    {
      name: 'Lagavulin',
      slug: 'lagavulin',
      country: 'Scotland',
  ...
  const [macallan] = await db.select().from(distilleries).where(
    // @ts-ignore
    ({ slug }) => slug === 'macallan'
  )
  const [talisker] = await db.select().from(distilleries).where(
    // @ts-ignore
    ({ slug }) => slug === 'talisker'
  )
  ...
  const [lag16Bottle] = await db.insert(bottles).values([
    { distilleryId: lagavulin[0].id, name: '16 Year Old', slug: 'lagavulin-16', type: 'scotch', category: 'single_malt' },
    { distilleryId: lagavulin[0].id, name: '8 Year Old', slug: 'lagavulin-8', type: 'scotch', category: 'single_malt' },
    { distilleryId: lagavulin[0].id, name: 'Distillers Edition', slug: 'lagavulin-de', type: 'scotch', category: 'single_malt' },
  ]).returning()
